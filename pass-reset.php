    ...
    <div class="row">
        <div class="col-md-12 py-5 mx-auto col-lg-6 col-sm-12">
            <div class="card shadow-lg p-4">
            	<div class="mt-4">
                    <h3 class="text-dark">Password Reset</h3>
                    <p class="text-primary">Fill in the details below to craete new password </p>
                    <form action="" name="form1" method="post">
                        ...
                        <input  type="submit" name="submit" class="btn btn-flat" value="Submit">
                            <div class="alert alert-danger" id="errormsg" style=" display:none; margin-top:8px;">
                                <strong>Invalid</strong> both password don't match
                            </div>
	                </form>
                </div>
            </div>
...
if (isset($_POST['submit'])) {
    ...
    if ($password != $cpassword) {
        ...
        $sql = mysqli_query($conn,"UPDATE providers SET password = '$hash' WHERE email='$email'")or die(mysqli_error($conn));
        if ($sql) {
            $_SESSION["user"]=$email;
            ?>
            <script type="text/javascript">
                window.location="success_pass.php";
            </script>
            <?php
        }
    }    
